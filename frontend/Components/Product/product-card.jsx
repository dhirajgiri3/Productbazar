...
const ProductCard = React.memo(function ProductCard({ product, viewMode = "grid" }) {
  ...
  if (isGridView) {
    ...
    return (
      <motion.div
        whileHover={{ y: -5 }}
        className="min-w-[280px] w-full bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-all duration-300"
      >
        ...
        <div className="p-4">
          ...
          {tags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              {tags.length > 2 && (
                <span className="text-xs text-gray-500 px-1">+{tags.length - 2}</span>
              )}
            </div>
          )}
          ...
          <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
            ...
            <Link
              href={`/product/${slug}`}
              className="text-xs font-medium text-violet-600 hover:text-violet-800 transition-colors flex items-center"
            >
              View Details
    ...
    return (
      <motion.div
        whileHover={{ y: -5 }}
        className="w-full bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-all duration-300"
      >
        <div className="flex">
          {/* Image Section */}
          <Link href={`/product/${slug}`} className="block w-1/4 min-w-[120px]">
