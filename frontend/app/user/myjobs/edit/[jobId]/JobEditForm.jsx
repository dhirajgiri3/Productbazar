...
import RichTextEditor from "@/app/jobs/post/components/RichTextEditor";
import SkillsInput from "@/app/jobs/post/components/SkillsInput";

const JobEditForm = ({ job }) => {
  ...
  const [logoPreviewUrl, setLogoPreviewUrl] = useState(job?.company?.logo || null);
  ...
  // Helper function to convert array to HTML for rich text editor
  ...
  const {
    ...
    defaultValues: {
      ...
      requirements: arrayToHtml(job?.requirements) || "",
      responsibilities: arrayToHtml(job?.responsibilities) || "",
      ...
      benefits: arrayToHtml(job?.benefits) || "",
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: (files) => {
      if (files[0]) {
        ...
        setLogoPreviewUrl(URL.createObjectURL(files[0]));
      }
    },
  });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const updatedJob = await updateJob(job._id, formData, companyLogo);
      ...
      <AnimatePresence mode="wait">
        <div className="max-w-4xl mx-auto py-10">
          ...
          >
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-3">Job Updated!</h1>
