...
const getRoleCapabilities = (role, secondaryRoles = []) => ({
  canUploadProducts: ['startupOwner', 'maker'].includes(role) || secondaryRoles.some(r => ['startupOwner', 'maker'].includes(r)),
  canInvest: role === 'investor' || secondaryRoles.includes('investor'),
  canOfferServices: ['agency', 'freelancer'].includes(role) || secondaryRoles.some(r => ['agency', 'freelancer'].includes(r)),
  canApplyToJobs: role === 'jobseeker' || secondaryRoles.includes('jobseeker'),
  canPostJobs: ['startupOwner', 'agency'].includes(role) || secondaryRoles.some(r => ['startupOwner', 'agency'].includes(r)),
  canShowcaseProjects: ['startupOwner', 'agency', 'freelancer', 'jobseeker'].includes(role) || 
...
export const AuthProvider = ({ children }) => {
  ...
  const determineNextStep = useCallback((userData) => {
    ...
    const { isEmailVerified = false, isPhoneVerified = false, isProfileCompleted = false } = userData;
    ...
    const progress = {
      ...
      percentage: Math.round((allSteps.filter(step => step.completed).length / allSteps.length) * 100)
    };

    // Return the first incomplete required step
    if (userData.email && !isEmailVerified) {
      return { type: "email_verification", title: "Verify your email address", description: "Verify your email for account security", required: true, allSteps, progress };
    }
    if (userData.phone && !isPhoneVerified) {
      return { type: "phone_verification", title: "Verify your phone number", description: "Add an extra layer of security", required: true, allSteps, progress };
    }
    if (!isProfileCompleted && !skippedSteps.includes("profile_completion")) {
      return { type: "profile_completion", title: "Complete your profile", description: "Help others know more about you", required: false, skippable: true, allSteps, progress };
    }
    return null;
  }, []);
  ...
  const handleUserData = useCallback((userData) => {
    ...
    if (hasSignificantChanges || !nextStep) {
      ...
      nextStepData ? localStorage.setItem("nextStep", JSON.stringify(nextStepData)) 
  ...
  const handleAuthSuccess = useCallback(
    (data) => {
      ...
      if (userData) {
        handleUserData(userData); // This will also call determineNextStep
      } else if (token) {
        // If user data is not directly in response and token exists, fetch user data
        fetchUserData(token);
      }
      
      // Navigation can be handled by individual functions or based on nextStep
      // For now, let's assume nextStep will guide navigation or it's handled elsewhere
    },
    [handleUserData, fetchUserData, setAccessToken] // Added dependencies
  );
